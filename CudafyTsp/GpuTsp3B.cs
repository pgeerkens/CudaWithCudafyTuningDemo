...
namespace CudafyTuningTsp {
	public class GpuTsp3_DivisorsCachedGlobal : AbstractTsp {
		public static readonly LatLongStruct[] _latLong	= new LatLongStruct[_cities];
		static GpuTsp3_DivisorsCachedGlobal() {
		...
		[Cudafy]
		public static int[] gpuDivisors	= new int[_cities];
		...
		internal override Answer GetAnswer() {
			...
			using (var gpu			= CudafyHost.GetDevice()) { 
				...
				var divisors		= new int[_cities];
				int divisor		= (int)_permutations;
      ...
      public static void GpuFindPathDistance(GThread thread, 
         ...
         while (permutation < permutations) {
            var distance = FindPathDistance(thread, permutations, permutation, path, gpuLatLong);
      ...
      public static float FindPathDistance(GThread thread, 
			int  permutations, int  permutation, int[,] path, LatLongStruct[] gpuLatLong) {
      ...
      public static float PathFromRoutePermutation(GThread thread, 
			int  permutations, int  permutation, int[,] path) {
         ...
         for (int city = _cities; city > 0; /* decrement in loop body */) {
            int divisor		= gpuDivisors[city-1];
            int dest			= (permutation / divisor) % city;
				city--;
            int swap									= path[dest, thread.threadIdx.x];
